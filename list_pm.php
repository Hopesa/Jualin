...
$dataid = mysql_fetch_assoc($query);
    $_SESSION['userid'] = $dataid["id_user"];
//We list his messages in a table
//Two queries are executes, one for the unread messages and another for read messages
$req1 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id_user as userid, users.username from message as m1, message as m2,users where ((m1.user1="'.$_SESSION['userid'].'" and m1.user1read="no" and users.id_user=m1.user2) or (m1.user2="'.$_SESSION['userid'].'" and m1.user2read="no" and users.id_user=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
$req2 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id_user as userid, users.username from message as m1, message as m2,users where ((m1.user1="'.$_SESSION['userid'].'" and m1.user1read="yes" and users.id_user=m1.user2) or (m1.user2="'.$_SESSION['userid'].'" and m1.user2read="yes" and users.id_user=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc')or trigger_error("Query Failed: " . mysql_error());;
?>
