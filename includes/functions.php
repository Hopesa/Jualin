...
************/
function createAccount($pUsername, $pPassword, $pEmail, $pTelp, $pAdd, $pCity, $pProv, $pNat) {
	...
	if (!empty($pUsername) && !empty($pPassword)) {
		...
		if ($uLen <= 4 || $uLen >= 11) {
			...
			$sql = "INSERT INTO users (`username` ,`email`, `password`) VALUES ('" . $eUsername . "', '" . $pEmail . "', '" . hashPassword($pPassword, SALT1, SALT2) . "');";
			$query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error());
             $sql = "SELECT id_user FROM users WHERE username = '$eUsername'";
 $query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error());
 $data = mysql_fetch_assoc($query);
    $id_user = $data["id_user"];
			$sql = "INSERT INTO alamat(`id_user`, `jalan`, `kota`, `provinsi`, `negara`, `no_telp`) VALUES ('" . $id_user . "','" . $pAdd . "','" . $pCity . "','" . $pProv . "','" . $pNat . "','" . $pTelp . "');";
            $query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error());
			if ($query) {
				return true;
			}	
