...
$dataid = mysql_fetch_assoc($query);
    $_SESSION['id_user'] = $dataid["id_user"];
$form = true;
$otitle = '';
$orecip = '';
$omessage = '';
...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
	{
		...
		if($dn1['recip']==1)
		{
			...
			if($dn1['recipid']!=$_SESSION['id_user'])
			{
				$id = $dn1['npm']+1;
				//We send the message
				if(mysql_query('insert into message (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['id_user'].'", "'.$dn1['recipid'].'", "'.$message.'", "'.time().'", "yes", "no")'))
				{
?>
